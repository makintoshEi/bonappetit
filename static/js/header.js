...
function tabChange(opcion){
	var padre=opcion.parentNode;
	var opciones = padre.getElementsByTagName("a");
	var i;
	for (i = 0; i < opciones.length; i++) {
		opciones[i].className = "";
	}
	opcion.className = "active";
	var tabs=["registro", "login"];
	for(i=0; i<tabs.length; i++)
	{
		document.getElementById(tabs[i]).className="tab";
	}
	document.getElementById(opcion.name).className="col-xs-12";
	document.getElementById(opcion.name).className+=" tab";
	document.getElementById(opcion.name).className+=" active";
}
