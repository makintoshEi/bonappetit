 ...
 */
 $("#frmMdFood").on("submit",function(event){
	...
	$.ajax({
		type: "POST",
		url: "/bebida/edit_drink/?trId="+$("#spId").attr('data-toggle'),
		data: formData,
		dataType: 'json',
		//necesario para subir archivos via ajax
		cache: false,
		...
		success: function(response) {
			if(response){
				$('#tbBebidas').DataTable().ajax.reload();
				$("#frmMdFood input").val('');
 ...
 */
 $.deleteFood = function(){
	$.ajax({
		type: "POST",
		url: "/bebida/delete_drink/",
		dataType: 'json',
		data: {id:trIdMd},
		success: function(response) {
			if(response){
				$.successMessage();
				$('#tbBebidas').DataTable().row( $("#"+trIdMd) ).remove().draw();
			}else{		
				$.errorMessage();
			}
		},
...
var btnsOpTblFoods = "<button style='border: 0; background: transparent' onclick='$.editDeleteFood(this, true);'>"+
	...
	$("#ltFood").click(function(event){
		...
		if (flagMd){
			$.fnTbl('#tbBebidas',"/bebida/get_drink_all/",[{ "data": "prd_nom"},{"data":"prd_prc"},{"data":"ctb_nom"},{"data":"tpr_des"}],$.renderizeRowTbModels);
			flagMd = false;		
		}
		else if(create){
			$('#tbBebidas').DataTable().ajax.reload();
