...
class Plato extends Private_Controller {
	...
	public function start()
	{
		...
		$user_data['categorias'] = $this->restaurant->selectSQLMultiple("SELECT * FROM categoria_plato",null);
		$this->load->view('user/platos',$user_data);
	...
	function do_upload($ci,$name,$edt=false)
	{
	    ...
	    if($edt)
		{	
			$dir = opendir('./uploads/food/');
			while ($file = readdir($dir)) 
			{ 
				if($file != '.' && $file != '..' && $file!=".DS_Store")
				{
					if (strpos($file, $ci)!==false){
						unlink('./uploads/food/'.$file);
					}
				}
			} 
			closedir($dir);
		...
		for($i=0; $i<$cpt; $i++)
	    {
			...
			$new_name = $ci.'.'.$tipo[1];
			//move_uploaded_file($_FILES[$name]['tmp_name'], './uploads/food/'.$new_name);
			...
			imagejpeg($lienzo, './uploads/food/'.$ci.'.jpg', 80);
	    }
		if ($cpt>1)
		{
			return true;
	...
	public function delete_food()
	{
		...
		if ($this->input->is_ajax_request()) 
    	{
			...
			$response = $this->platos->delete($data);
	...
	public function save_food()
	{
		...
		if ($this->input->is_ajax_request()) 
    	{
			...
			$response = $this->restaurant->selectSQL("SELECT insert_plato(?,?,?,?,?,?,?)",$data);
			
			if($response->insert_plato!='0')
			{
				$new_name = $response->insert_plato.'_'.do_hash($response->insert_plato, 'md5');
				
				$this->do_upload($new_name, 'images');
			}
			
			echo json_encode($response->insert_plato);
		}
		else
		{
			exit('No direct script access allowed');
	...
	public function get_food_all()
	{
    	...
    	{
			$user_data = (array)$this->session->userdata('logged_user');
    		$data = $this->restaurant->selectSQLMultiple('Select * from food_view where rst_id='.$user_data["rst_id"],null);
	...
	public function find_food()
	{
		...
		if ($this->input->is_ajax_request()) 
    	{
    		$data = $this->restaurant->selectSQLMultiple('Select * from food_view where prd_id=?',array($this->input->post('id')));
			echo json_encode(array("data"=>$data));
		}
		else
		{
	...
	public function edit_food()
	{
		...
		if ($this->input->is_ajax_request()) 
    	{
			...
			$response = $this->platos->update($this->input->get('trId'),$data);
			echo json_encode($response);
		}
		else
		{
